...
DebugProgramFactory.SetupSymbols = function(program) {
	...
	$(program.$window).find(".memory-link").each(function(){
		...
		if( symbol ) {
			new Tooltip(this, symbol.label);
		}
	});

	$(program.$window).find(".execution-link").each(function(){
		var hex_address = this.innerText;
		var address = parseInt(hex_address,16);
...
DebugProgramFactory.GetProgram = function(type) {
	...
	switch( type ) {
		...
		case 'symbol':
			program = new DebugSymbolProgram(emulation_core);
			break;

		case 'trace':
			program = new DebugTraceProgram(emulation_core);
			break;
...
var DebugWriteMemory = function(address, values) {
	...
	for( var i = 0; i < values.length; i++ ) {
		gameboy.memory[parseInt(address)+i] = values[i];
	}
}
